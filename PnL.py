...
def profit_loss(market='BNB-BTC', start_date='2022-12-01', end_date='2022-12-31', client=Client(), showlog=False):
    ...
    try:
        ...
        # Create DataFrame
    df = pd.DataFrame(trades, columns=['time', 'symbol', 'isBuyer', 'price', 'qty', 'quoteQty', 'commission',
                                       'commissionAsset'])
    ...
    if asset_quote == 'BTC':
        ...
        try:
            btc_price = float(
                client.get_klines(symbol=asset_quote + 'BTC', interval='1m', startTime=prices_time, limit=1)[0][4])
        except:
            print(f"Something wrong with the request of {asset_quote}BTC price. Please try again.")
    ...
    if asset_quote == 'BNB':
        ...
        try:
            bnb_price = float(
                client.get_klines(symbol='BNB' + asset_quote, interval='1m', startTime=prices_time, limit=1)[0][4])
        except:
    ...
    if showlog:
        ...
        print(f"   Price {asset_quote}BTC: {btc_price}")
    ...
    return {
        ...
        'quote_price_end_of_period': btc_price,
...
pnl = profit_loss(market='BNB-BTC', client=client, showlog=True)
print(pnl)
